...
* **Step 8** — Model control workers (`model_control_workers.*`): per-kernel idle→busy→idle FSM (bits 0 `ap_start` / 1 `ap_done`) + clock-wizard lock-bit pin, over `ModelClient` against injected BAR memfds. Two address spaces per kernel (see invariant 8): the BAR memfd is addressed by `base_address % kUserRegionSize` (BAR-window offset), the model by the absolute `base_address`; register offsets add onto each. (An earlier version conflated them — using the absolute base as a memfd offset — see the Step-14 kernel-finishing fix.)
...
8. **Kernel base address = two spaces (mirrors VRT `vrt/src/kernel.cpp`).** A kernel's `base_address` in the system map is an ABSOLUTE AXI address (e.g. `0x20200010000`), far past the 128 MiB user-region BAR. `KernelWorker` splits it: `bar_base = base_address % kUserRegionSize` for every BAR-memfd access (the BAR window the user/VRT sees; hardware path `barOffset = absoluteAddr % barLen`), and `model_base = base_address` for every `ModelClient` `reg_write`/`fetch_scalar` (the simulation path `sendScalar(baseAddr + offset, ...)`). Register offsets add onto each. Conflating them — using the absolute base as a memfd offset — makes every memfd access out-of-range, so the worker goes `Dead` on its first poll before ever seeing `ap_start`, and the kernel silently never runs.
